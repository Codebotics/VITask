...
def getToken():
    ...
    if request.headers.get('X-VITASK-API') != "2020_Mar_25" or request.args.get("username") is None or request.args.get("password") is None:
        return jsonify({
...
def sync():
    ...
    refresh = request.args.get("hardRefresh")
    ...
    if refresh is None or not refresh:
        ...
        attendance,_ = get_attandance(sess, username, key)
        marks = get_marks(sess, username, key)
        return jsonify({
        ...
        attendance,_ = get_attandance(sess, username, key)
        marks = get_marks(sess, username, key)
        acadHistory = get_acadhistory(sess,username,key)
        days = get_timetable(sess, username, key)
        return jsonify({
            "attendance": attendance,
            "marks" : marks,
...
def moodleLogin():
    ...
    ref = db.reference('vitask')
    # For the last time BASE64 IS NOT ENCRYPTION
...
def moodleSync():
    ...
    if(temp is None):
        ...
        session['id'] = key
        b64_username = ref.child("moodle").child('moodle-'+session['id']).child(key).child('Username').get()
        b64_password = ref.child("moodle").child('moodle-'+session['id']).child(key).child('Password').get()
        # Now first decode the usernames and password
        b64_password = b64_password.encode('ascii')
        b64_username = b64_username.encode('ascii')

        password = base64.b64encode(b64_password).encode('ascii')
        username = base64.b64encode(b64_username).encode('ascii')
...
def assignmentToggleShow():
    ...
    if request.headers.get('X-VITASK-API') != "2020_Mar_25" or request.args.get("ids") is None:
        return jsonify({
            "error" : "Incorrect API Request",
            "code"  : "400" # Bad request
        })
    ...
    for i in range(assignments):
        if assignments[i]['id'] in request.args.get("ids"):
